...
void TatpBatchProcessor::clearBatch() {
...
uint32_t TatpBatchProcessor::hash_int(uint32_t a) {
	a = (a+0x7ed55d16) + (a<<12);
	a = (a^0xc761c23c) ^ (a>>19);
	a = (a+0x165667b1) + (a<<5);
	a = (a+0xd3a2646c) ^ (a<<9);
...
void TatpBatchProcessor::processBatch() {
	...
	std::vector<std::thread> dcf_sid_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		dcf_sid_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
	...
	std::vector<std::thread> icf_sid_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		icf_sid_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
	...
	std::vector<std::thread> ul_sid_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		ul_sid_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
	...
	std::vector<std::thread> gad_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		gad_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
	...
	std::vector<std::thread> dcf_icf_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		dcf_icf_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
	...
	std::vector<std::thread> gsd_ul_usd_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		gsd_ul_usd_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
	...
	std::vector<std::thread> icf_usd_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		icf_usd_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
