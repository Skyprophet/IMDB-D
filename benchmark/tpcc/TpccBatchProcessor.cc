...
void TpccBatchProcessor::clearBatch() {
...
uint32_t TpccBatchProcessor::hash_int(uint32_t key) {
...
void TpccBatchProcessor::processBatch() {
	...
	std::vector<std::thread> no4_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		no4_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
	...
	std::vector<std::thread> no12_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		no12_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
	...
	std::vector<std::thread> no5_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		no5_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
			for (size_t i = 0; i < new_order_params_i_id_bkt[t].size(); ++i) {
				auto entry = new_order_params_i_id_bkt[t].at(i);
	...
	std::thread payment_txn1_thread([&]() {
		Timer timer;
		timer.start();
	...
	std::thread payment_txn2_thread([&]() {
		Timer timer;
		timer.start();
		for (size_t i = 0; i < payment_params.size(); ++i) {
	...
	std::thread payment_txn3_thread([&]() {
		Timer timer;
		timer.start();
		for (size_t i = 0; i < payment_params.size(); ++i) {
	...
	for (auto && entry : no12_thds)
		entry.join();
	//new_order_txn12_thread.join();

	std::vector<std::thread> neworder_conflict_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		neworder_conflict_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
	...
	std::vector<std::thread> del3_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		del3_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
	...
	std::vector<std::thread> no6_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		no6_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
			for (size_t i = 0; i < new_order_params_bkt[t].size(); ++i) {
				size_t pos = new_order_params_bkt[t].at(i);
	...
	std::vector<std::thread> del2_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		del2_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
			for (size_t i = 0; i < delivery_params_bkt[t].size(); ++i) {
				auto && entry = delivery_params_bkt[t].at(i);
	...
	std::vector<std::thread> conflict2_thds;
	for (size_t t = 0; t < kParallelism; ++t) {
		conflict2_thds.emplace_back(std::thread([&](const size_t& t) {
			Timer timer;
			timer.start();
	...
	payment_txn1_thread.join();
	payment_txn2_thread.join();
	payment_txn3_thread.join();
